...
- Once you've obtained the sample app you should open it in VSCode and then start the app.
  ...
  - Once you've got the Terminal panel open you'll need to install all the modules that the app needs.
    ...
    This _should_ install everything that's needed but be warned that this may take a while - it'll need to download about 30,000 separate files that total up to about 200 MB of space. This step is easy, but not quick
...
- Everything that's "inside" is between the opening { and the matching, closing }. VSCode helps you match these - put your cursor on the opening { and VSCode will draw a box around it. If you scroll down far enough you'll see the matching } which VSCode has drawn a matching box around. By looking for the boxes VSCode can help you figure out which curly braces are matching which others.
  ...
  ![matchingBrackets](images/matchingBrackets.png)

- The function returns a single HTML <div> element, that has been styled using the "App-header" CSS class. Inside of that we want React to render the <TodoParent> element, which will handle everything about rendering the todo list, responding to buttons being clicked, etc.
...
- Note that the `todoArray` object is a special instance variable that React looks for. When this changes React will re-render the page. We will need to tell React when it changes
...
- Point out that `todoArray` is actually an array of the individual items
...
- it starts by "deconstructing" (unpacking) the props into local variables (local constants)
...
- We also use the fat arrow syntax to create a new function to toggle the state of a particular item.
  - `setTodoArray()` changes the state in the TodoParent object NOT on the component that calls markComplete
...
- We keep track of the user's input via an instance variable which stores a reference (a `ref`) to the textbox's DOM object
  ...
  - We create the `ref` via this line of code : `let input = React.useRef(null);`
...
- The `<input type="text">` tells the browser to put a text box on the page (as opposed to a button, a checkbox, etc).
  ...
  - `ref={input}` tells React that when the textbox is created to establish a reference between the input instance variable and the DOM node (i.e., the textbox)
...
One common error is to try and change state (todoArray here) directly. Syntactically you're allowed to do this (in other words,the program will compile and run) but semantically this is wrong. What will happen is that we'll change the app's state but React will not know that it's changed and so the screen that the user is looking at won't change. **We must call the setTodoArray method instead**.
