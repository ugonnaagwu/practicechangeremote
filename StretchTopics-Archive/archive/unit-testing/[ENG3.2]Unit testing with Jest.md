...
## Your testing experience and background (10 minutes)

* Your experience with testing
 
Think about your testing experience. How do you typically test the code that you write?

* Videos

  * Watch this [video](https://www.youtube.com/watch?v=5mesy72RATs) about unit testing and answer the following questions.

    * What is unit testing?
    * Who does unit testing?
    * When would you do unit testing?
    * Why is it important?
    * How do you do unit testing?
    * How do passing and failing tests appear?
    * What is test-driven development?
    * What is integration testing?
 
  * Watch the series of [videos](https://www.youtube.com/watch?v=D9DdY2WmM-s) about Jest.

* Testing in the real world

Interview your coaches to learn about how testing of large React applications is done. Who is doing the testing? How the interaction between developers and testers is happening? What tools are used for testing? 

## What is unit testing? (10 minutes)
...
* Cite some differences between unit testing, integration testing, and functional testing?
...
Source: [Image](https://www.codecademy.com/articles/tdd-red-green-refactor)

## Jest (10 minutes)

Jest is an open JavaScript testing library developed by Facebook. While Jest can be used to test any JavaScript library, it is very popular when it comes to React 
...
## Run the tests of the sample code
...
Check out `package.json` for the dependencies.

#### Run Jest

1. A file `App.test.js` is created automatically by create-react-app to test `App.js`. For each file `file.js` under test, we can create a file `file.test.js`
2. Organize your tests by choosing relevant names and using directories
3. In the Terminal window, use `npm t` or `npm test` to run the test

#### Troubleshooting

It may happen that you get an error when running `npm t`. In that case, the list of actions/steps to take to solve the problem are provided. The problem comes from dependencies between installed packages. Info is available [here](https://stackoverflow.com/questions/56528222/npm-start-returns-error-there-might-be-a-problem-with-the-project-dependency-t). By following the steps you should be able to solve the problem.
...
* `test` is a function that takes a string and a function as parameters. The overall syntax of test is `test("", () => {expect().matcher()})`. 
  * The first parameter (a string) is the title of the test. 
  * The second parameter is a function that contains the expectations to test. That part uses a matcher that matches the actual value with the expected value.  A complete description of `test` is available [here](https://jestjs.io/docs/en/api#testname-fn-timeout). 
* The number of calls of `test` determines the number of test cases
...
* Often you have some setup work that needs to happen before running tests and some finishing work that needs to happen after running tests. Jest provides helper functions to handle this:`beforeEach` and `afterEach`
...
inish writing unit tests related to the UI for the 'unit-testing-with-jest' project

Write unit tests related to the YourShare project. For example, you can test that clicking on a button brings you to the correct page.

Learn more about Jest from the videos that start here
